...
**Response**: There are La Quinta's located outside of the US, including places like New Zealand, Turkey, and Canada. On the other hand, Denny's is only located in the US (and oddly enough not in every state).
...
**Response**: To get just the US locations, Denny's is all set but we would need to filter the rows with just US states (and dropping the other locations).
...
```{r Checking for outside US Dennys with filter}
...
```{r Checking for outside US Dennys with anti_join}
dennys %>% 
